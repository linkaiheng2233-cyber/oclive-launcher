...
- **工程**：全仓 **`cargo fmt`**；**`npm run check`**（`build` + `fmt --check` + **`clippy -D warnings`** + `cargo test`）；修复 Clippy（`map_or` → `is_some_and`、`io::Error::other`、子进程日志行 **`map_while(Result::ok)`**）。
- **界面**：日间 **暖色主题**（象牙/卡其）；顶栏 **界面缩放**（A− / 百分比 / A+，与编写器同档位）；**版本与下载** 等页 **HelpHint** 大白话说明；顶栏 **问号** 解释字号、主题与「保存配置」；若干 **z-index** 修正避免问号被遮挡。
- **文档**：README 补充 **性格档案分工**（核心在包内、可变在运行时 DB、`personality_source`）并链到 oclivenewnew `personality-archive-notes.md`、`design-axis-evolution.md`、`README_MANIFEST`；编写器页 HelpHint 文案与之一致；`docs/announcements/developer.md` 含简要链到设计轴心文档。
